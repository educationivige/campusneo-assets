(function () {
    ...
    const PNT_DEFAULT_IMAGE = 'https://ivirmacampus.com/pluginfile.php/1/local_uploadfiles/additionalimages/0/default.png';
    ...
    function getTitle(card) {
        const el = card.querySelector('.tw-catalogItemNarrow__title');
    ...
    function isLfe(title) {
        return title.toUpperCase().includes('LFE');
    }
    ...
    function extractIdNetex(card) {
        ...
        // Si ya fue procesado antes, el valor estará en el atributo
    ...
    function procesarCard(card) {
        ...
        if (imageEl) {
            let imageUrl;
            if (isPnt(title)) {
                imageUrl = resolvePntImage(title);
            } else if (idNetex) {
                imageUrl = `${NETEX_IMAGE_BASE}${idNetex}_dashboard.jpg`;
            }
        ...
        if (isLfe(title)) {
            ...
            if (imageWrap && !imageWrap.querySelector('.lfe-badge')) {
    ...
    function observar() {
        ...
        const observer = new MutationObserver(function (mutations) {
            ...
            mutations.forEach(function (m) {
                ...
                // Script externo sobrescribió el background-image
                if (
                    m.type === 'attributes' &&
                    m.attributeName === 'style' &&
                    m.target.classList.contains('tw-catalogItemNarrow__image_ratio_img')
                ) {
    ...
    function init() {
        ...
        // Reintentos por renderizado asíncrono de Totara
        [500, 1200, 2500].forEach(ms => setTimeout(procesarTodo, ms));
    }
